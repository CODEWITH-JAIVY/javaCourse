...
> A stream is **not a data structure**. It is a **computation pipeline** — a recipe for processing data. It does not
> store any elements.
...
Both produce the same result. The stream version is more readable, composable, and can be parallelized with one word
change.
...
length() >4) // intermediate — lazy, not run yet
        .
...
> ⚠️ **Critical Rule:** Without a terminal operation, **nothing runs**. Intermediate operations are just a description,
> not an execution.
...
Passes each element through a **YES/NO test**. Elements where the test returns `true` pass through. Elements where it
...
filter(longerThan3.and(startsWithA));

// OR — either condition must be true
        stream.

filter(longerThan3.or(startsWithA));

// NEGATE — flip the condition
        stream.

filter(longerThan3.negate()); // length <= 3
...
Transforms **every element** into a new value. Every element in → exactly one element out. Count never changes. But the
**type** can change completely.
...
mapToLong(User::getId)          // Stream<User>   → LongStream
stream.
...
Terminal operations **trigger the entire pipeline to execute**. They are eager — once called, stream processing runs
from source to terminal in a single pass.

> ⚠️ After a terminal operation, the stream is **consumed and closed**. Calling another terminal op throws
...
orElse("default"); // safe — returns default if empty
...
println("Pipeline created — nothing printed yet!");
...
println(result); // [PRIYA, JAMES, ROHAN]
```
...
ifPresent(System.out::println);     // Sameer
...
println(sentence); // "java stream api cool"
```
...
count();                      // ❌ IllegalStateException: stream already operated upon
...
add(s));    // ❌ mutating external state — unsafe in parallel
...
stream(array)                   // from array
Stream.

of(a, b, c)                    // from values
IntStream.

range(start, end)           // int range (exclusive end)
IntStream.

rangeClosed(start, end)     // int range (inclusive end)
Stream.

generate(Supplier)             // infinite stream
Stream.

iterate(seed, UnaryOperator)   // infinite sequence
```
...
> **Next topics to study:** `flatMap()` → `sorted()` + `distinct()` → Advanced Collectors (`groupingBy`,
`partitioningBy`) → `Optional` in depth → Primitive Streams → Parallel Streams → Custom Collectors